...
{
    [Serializable]
    public class EntityNotReceived : BasicLuisDialog
    {
        static Constants constants = new Constants();
        public async Task Booking(IDialogContext context)
        {
            string prompt = "Please select any one of the following";
            string retryprompt = "Please try again";
            var promptOptions = new PromptOptions<string>(prompt: prompt, options: constants.BookingOption, retry: retryprompt, speak: prompt, retrySpeak: retryprompt, promptStyler: new PromptStyler());
            PromptDialog.Choice(context, BookReceivedAsync, promptOptions);
        }
        public async Task BookReceivedAsync(IDialogContext context, IAwaitable<string> result)
        {
            string r = await result;
            if (r.ToLower().Equals("movie"))
            {
            ...
            else if (r.ToLower().Equals("flight"))
            {
            ...
            else if (r.ToLower().Equals("railway"))
            {
        ...
        public async Task Shopping(IDialogContext context)
        {
            string prompt = "Please select any one of the following";
            string retryprompt = "Please try again";
            var promptOptions = new PromptOptions<string>(prompt: prompt, options: constants.ShoppingSite, retry: retryprompt, speak: prompt, retrySpeak: retryprompt, promptStyler: new PromptStyler());
            PromptDialog.Choice(context, ShopReceivedAsync, promptOptions);
        }
        public async Task ShopReceivedAsync(IDialogContext context, IAwaitable<string> result)
        {
            string r = await result;
        ...
        public async Task PasswordChange(IDialogContext context)
        {
            string prompt = "Please select any one of the following";
            string retryprompt = "Please try again";
            var promptOptions = new PromptOptions<string>(prompt: prompt, options: constants.PasswordOption, retry: retryprompt, speak: prompt, retrySpeak: retryprompt, promptStyler: new PromptStyler());
            PromptDialog.Choice(context, PassReceivedAsync, promptOptions);
        }
        public async Task PassReceivedAsync(IDialogContext context, IAwaitable<string> result)
        {
            string r = await result;
            ...
            {
                await context.PostAsync("What password do you want to change?");
                context.Wait(PasswordOptionReceived);
            }
        }
        public async Task PasswordOptionReceived(IDialogContext context, IAwaitable<IMessageActivity> result)
        {
            var password = await result;
            string str = password.Text.Replace(" ", String.Empty);
            await context.PostAsync("I can search the web for that");
            await context.SayAsync(text: $"Search Link: http://www.google.com/search?q=how+to+change+the+password+of+"+str, speak: "Click the link to see the result in web");
            await new CloseContact().ConversationEnding(context);
        }
        public async Task Installation(IDialogContext context)
        {
            string prompt = "Please select any one of the following";
            string retryprompt = "Please try again";
            var promptOptions = new PromptOptions<string>(prompt: prompt, options: constants.SoftwareOption, retry: retryprompt, speak: prompt, retrySpeak: retryprompt, promptStyler: new PromptStyler());
            PromptDialog.Choice(context, SoftwareReceivedAsync, promptOptions);
        }
        public async Task SoftwareReceivedAsync(IDialogContext context, IAwaitable<string> result)
        {
            string r = await result;
            if (r.ToLower().Contains("other"))
            {
                await context.PostAsync("Please mention the name of the software:");
                context.Wait(SoftwareNameReceived);
            }
            else
            {
        ...
        public async Task SoftwareNameReceived(IDialogContext context, IAwaitable<IMessageActivity> result)
        {
            var software = await result;
            await context.PostAsync($"Request to install '{software.Text}' is received.");
            string str = software.Text.Replace(" ", String.Empty);
            await context.SayAsync(text: $"Search Link: http://www.google.com/search?q=download+str", speak: "This is the link where you can install the software");
            await new CloseContact().ConversationEnding(context);
        }
    }
}